...
<?php  if($syncStatus){ ?>
	<div class="tips_light">敏感词已更新到<?php echo htmlspecialchars(Pw::time2str($sync['lasttime'], 'Y-m-d'), ENT_QUOTES, 'UTF-8');?>，新增<?php echo htmlspecialchars($sync['syncnum'], ENT_QUOTES, 'UTF-8');?>个词语，<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=sync'; ?>">立即同步</a></div>
<?php  } ?>
	<div class="h_a">提示信息</div>
	<div class="prompt_text">
		<ul>
			<li>敏感词管理可以有效保证您网站的顺利运营。为了提高发帖效率，建议敏感词不超过3500个</li>
			<li>您可以&nbsp;<a target="_blank" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=export'; ?>" class="mr10">导出词库</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=import'; ?>" class="J_dialog" title="导入本地词库">导入本地词库</a></li>
		</ul>
	</div>
	<form id="J_word_tip_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=setconfig'; ?>" method="post">
	...
	<form method="post" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=search'; ?>">
  ...
  $selected = $key == $args[type] ? 'selected' : ''; ?>
	...
	<div class="mb10"><a class="btn J_dialog" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=add'; ?>" title="添加敏感词"><span class="add"></span>添加敏感词</a></div>
	...
	<form id="J_word_form" class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=%2A'; ?>" method="post">
  ...
  $selected = $key == $args[type] ? 'selected' : ''; ?>
			...
			<?php  } ?>
				...
				<?php  foreach ($wordList as $key => $value) {
					$value['word_type'] = Pw::inArray($value['word_type'], array_keys($typeList)) ? $typeList[$value['word_type']] : '';
				?>
				<tr>
					...
					<td><a class="mr5 J_dialog" title="编辑" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?id=', rawurlencode($value['word_id']),'&m=word&c=manage&a=edit'; ?>">[编辑]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=delete'; ?>" data-msg="确定删除该敏感词" class="J_ajax_del" data-pdata="{'id': '<?php echo htmlspecialchars($value['word_id'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></td>
				</tr>
				<?php  } ?>
				</tbody>
			</table>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=word&c=manage&a=search'; 
...
Wind.use('dialog', 'ajaxForm', function(){
	...
	var URL_DELETE = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=batchdelete'; ?>",	//删除
			URL_EDIT = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=batchedit'; ?>",	//编辑
	...
	$('#J_filter_select').on('change', function(){
		window.location.href = '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=word&c=manage&a=search'; ?>&type='+ $(this).val();
	});
