...
<div class="wrap J_check_wrap">
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck'; ?>">主题审核</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck&a=reply'; ?>">回复审核</a></li>
		</ul>
	</div>
	...
	<form method="post" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck'; ?>">
	...
	<form class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck&a=doPassThread'; ?>" method="post">
		<?php  if ($threadb) { ?>
		...
		<div class="table_list">
			<table width="100%">
				...
				<tr>
					...
					<td width="600">
						<div class="mb5">
							<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','thread/', rawurlencode($value['fid']),'.html'; ?>" class="mr5" target="_blank">[<?php echo $forumlist[$value['fid']]['name'];?>]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','articles/', rawurlencode($value['tid']),'.html'; ?>" target="_blank"><?php echo htmlspecialchars($value['subject'], ENT_QUOTES, 'UTF-8');?></a>
						</div>
						<div class="mb5">
							作者：<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','user/', rawurlencode($value['created_userid']),'.html'; ?>" target="_blank" class="mr15"><?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?></a>发帖时间：<?php echo htmlspecialchars(Pw::time2str($value['created_time']), ENT_QUOTES, 'UTF-8');?>
						</div>
					...
					<td>
						<ul class="single_list cc">
							<li><a class="J_ajax_refresh mr5" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck&a=doPassThread'; ?>" data-pdata="{'tid': '<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>'}">[通过]</a></li>
							<li><a class="J_ajax_del" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck&a=doDeleteThread'; ?>" data-pdata="{'tid': '<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></li>
						</ul>
					</td>
				</tr>
				<?php  } ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=bbs&c=contentcheck'; 
...
<?php } ?>
		...
		<button class="btn btn_submit J_ajax_submit_btn" type="submit">通过</button>
				<button class="btn J_ajax_submit_btn" type="submit" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=contentcheck&a=doDeleteThread'; ?>">删除</button>
			</div>
		</div>
		<?php  } else { ?>
