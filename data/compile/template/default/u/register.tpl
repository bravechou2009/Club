	...
	<div class="main_wrap">
		<div class="box_wrap register cc">
			...
			<div class="reg_cont_wrap">
				<div class="reg_cont">
					<form id="J_register_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-dorun'; ?>" method="POST">
					<div class="reg_form">
 ...
 if ($verify) {?>
			...
			<div class="reg_side">
				<div class="reg_side_cont">
					...
					<p><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-login-run?backurl=', rawurlencode($backurl); 
	...
	register_form.validate({
		...
		rules: {
			invitecode : {
				required	: true,
				remote		: {
					url : "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-checkInvitecode'; ?>",
					type : 'post',
					dataType: "json",
					data : {
						invitecode : function(){
			...
			username: {
				required	: true,
				remote		: {
					url : "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-checkusername'; ?>",
					type : 'post',
					dataType: "json",
			...
			password : {
				required: true,
				remote : {
					url : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-checkpwd'; ?>',		//验证密码
					dataType: "json",
					type : 'post',
					data : {
						username : function(){
			...
			mobileCode : {
				required : true,
				remote : {
					url : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-mobile-checkmobilecode'; ?>',		//验证手机验证码
					dataType: "json",
					type : 'post',
					data : {
						mobileCode : function(){
			...
			code : {
				required : true,
				remote : {
					url : "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','verify-index-check'; ?>",
					dataType: "json",
					type : 'post',
					data : {
						code :  function(){
			...
			if(id == 'password') {
				...
				$(element).on('keyup', function(e){
					...
					if(e.keyCode !== 9) {
						...
						$.post('<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-checkpwdStrong'; ?>', {
...
<?php if ($config['active.phone']) {?>
	//手机验证
	window.M_CHECK = '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-sendmobile'; ?>';
	window.M_CHECK_MOBILE = '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-checkmobile'; ?>';
