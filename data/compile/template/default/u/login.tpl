...
?>
			...
			<div class="reg_cont_wrap">
				<div class="reg_cont">
					<form id="J_u_login_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-login-dorun'; ?>" method="post" >
					<div class="reg_form">
						<?php if ($url) {?>
						...
						<dl class="cc pick">
							<dt>&nbsp;</dt>
							<dd><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-findPwd-run'; ?>" class="fr mr10">找回密码</a><input name="rememberme" value="31536000" type="checkbox" class="checkbox" id="cktime"><label for="cktime">自动登录</label></dd>
						</dl>
						<dl class="cc">
							<dt>&nbsp;</dt>
			...
			<div class="reg_side">
				<div class="reg_side_cont">
					...
					<p class="mb20"><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-run'; 
	...
	u_login_form.validate({
		...
		rules: {
			username: {
				...
				//nameCheck : true
				remote		: {
					url : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-login-checkname'; ?>',
			...
			password : {
				required	: true/*,
				remote : {
					url : "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-login-checkpwd'; ?>",
					dataType: "json",
					type : 'post',
					data : {
						username :  function(){
            ...
            */
			...
			answer : {
				required : true,
				remote : {
					url : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-login-checkquestion'; ?>',
					dataType: "json",
					type : 'post',
					ignoreRepeat : true,
					data : {
