...
<div class="tac"> </div>
		...
		<div class="box_wrap post_page">
			<nav>
				<div class="content_nav">
					<ul id="tabTypeHead">
					<?php  if ($action == 'reply') { ?>
						<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-reply?tid=', rawurlencode($tid); ?>">发表回复</a></li>
					<?php  } elseif ($action == 'modify') { ?>
						<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-modify?tid=', rawurlencode($tid),'&pid=', rawurlencode($pid); ?>">编辑帖子</a></li>
					<?php  } else { 
  foreach ($pwforum->getThreadType($loginUser) as $key => $value) { ?>
			...
			<div class="post_content">
				<a id="J_draft_lis_btn" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-draft-myDrafts'; ?>" class="my_drafts">草稿箱<span class="red" style="<?php  if (!$draftCount) { ?>display:none;<?php  } ?>">(<em id="J_draft_count"><?php echo htmlspecialchars($draftCount, ENT_QUOTES, 'UTF-8');?></em>)</span></a>
...
?>
 ...
 include("/data/home/qxu1098490259/htdocs/data/compile/template/default/bbs/wind_editor.tpl"); ?>
						</div>
					</div>
					<div class="sd">
						<div class="sidebar">
...
?>
				...
				<div class="ft cc">
					...
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-draft-doAdd'; ?>" class="btn btn_big mr10" id="J_draftBtn">存为草稿</a>
				...
				$.getJSON($this.attr('href'), function(data){
					if(data.state === 'success') {
						...
						$.each(data.data, function(i, o){
							li_arr.push('<li><a data-pdata="{\'id\':\''+o.id+'\'}" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-draft-doDelete'; ?>" class="pop_close J_draft_del" title="删除草稿">[删除]</a><a href="" data-id="'+ o.id +'" class="insert J_draft_insert" title="载入草稿">[载入]</a><span class="J_draft_title">'+ o.title +'</span><span>'+ o.created_time +'</span></li>');
