<?php if (!function_exists("TPL_SPACE_PROFILE_EXTEND_EDUCATION")) {function TPL_SPACE_PROFILE_EXTEND_EDUCATION($space){?>
				...
				<div class="space_profile">
					<h3>
						<?php  if ($space->tome == 2){ ?>
						<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','profile-education-run?_tab=education'; ?>" class="edit">编辑</a>
						<?php  } ?>
						<strong>教育经历</strong>
					</h3>
...
<?php if (!function_exists("TPL_SPACE_PROFILE_EXTEND_WORK")) {function TPL_SPACE_PROFILE_EXTEND_WORK($space){?>
				...
				<div class="space_profile">
					<h3>
						<?php  if ($space->tome == 2){ ?>
						<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','profile-work-run?_tab=work'; ?>" class="edit">编辑</a>
						<?php  } ?>
						<strong>工作经历</strong>
					</h3>
