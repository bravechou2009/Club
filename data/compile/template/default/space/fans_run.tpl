...
?>
  ...
  foreach ($fans as $key => $value) { ?>
					<dl class="cc">
						<dt><a data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>" class="J_user_card_show" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','user/', rawurlencode($value['uid']),'.html'; ?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($value['uid'], 'small'), ENT_QUOTES, 'UTF-8');?>" data-type="small" width="50" height="50" /></a><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','message-message-pop?username=', rawurlencode($value['username']); ?>" data-name="<?php echo htmlspecialchars($value['username'], ENT_QUOTES, 'UTF-8');?>" class="called J_send_msg_pop J_qlogin_trigger">打招呼</a></dt>
						<dd>
							<div class="title">
								<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','user/', rawurlencode($value['uid']),'.html'; ?>" data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>" class="name J_user_card_show"><?php echo htmlspecialchars($value['username'], ENT_QUOTES, 'UTF-8');?></a>
							...
							<div class="attribute">
								...
								?>
									<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','my-follow-add'; ?>" class="core_follow J_space_fans J_qlogin_trigger" data-followed="<?php echo htmlspecialchars($isfan, ENT_QUOTES, 'UTF-8');?>" data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>">加关注</a>
								<?php  } ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-fans-run?uid=', rawurlencode($space->spaceUid),'&page=', rawurlencode($_page_i); 
...
<?php } 
  ...
  if ($loginUser->uid == $space->spaceUid) { ?>
					...
					<?php  } else { ?>
					<!--看别人-->
					<div class="not_content">啊哦，Ta还没有粉丝，我来第一个<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','my-follow-add'; ?>" id="J_nofans_follow" data-uid="<?php echo htmlspecialchars($space->spaceUid, ENT_QUOTES, 'UTF-8');?>">关注Ta</a>！</div>
					<?php  } 
  } ?>
				</div>
			</div>
