...
<!--我的帖子-->
			<div class="box">
				<div class="my_article">
					<div class="hd"><h2 class="name"><?php echo htmlspecialchars($host, ENT_QUOTES, 'UTF-8');?>的帖子</h2><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-thread-run?uid=', rawurlencode($space->spaceUid); ?>" class="current">主题</a><span class="line">|</span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-thread-post?uid=', rawurlencode($space->spaceUid); ?>">回复</a></div>
					<div class="ct">
						<?php  if ($threads) { ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-thread-run?page=', rawurlencode($_page_i); 
