...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="read_pages_wrap cc">
			<?php  if($showReply){ ?>
			<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-reply?tid=', rawurlencode($tid); ?>" data-referer="true" class="btn_replay<?php echo htmlspecialchars($replyNeedLogin, ENT_QUOTES, 'UTF-8');?>">回复</a>
			<?php  } ?>
			<div class="pages" style="margin-right:3px;"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','thread/', rawurlencode($pwforum->fid),'-', rawurlencode($fpage),'.html'; ?>" class="pages_pre" rel="nofollow">&laquo; 返回列表</a></div>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','articles/', rawurlencode($tid),'-', rawurlencode($_page_i),'.html'; 
...
<?php } ?>
  ...
  if ($operateThread['topped']){ ?><a data-type="norefresh" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-topped'; ?>" class="">置顶</a><?php  } 
  if ($operateThread['digest']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-digest'; ?>" class="">精华</a><?php  } 
  if ($operateThread['highlight']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-highlight'; ?>" class="">加亮</a><?php  } 
  if ($operateThread['up']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-up'; ?>" class="">提前</a><?php  } 
  if ($hasFirstPart && $hasSecondPart){ ?><i>|</i><?php  } 
  if ($operateThread['type']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-type'; ?>" class="">分类</a><?php  } 
  if ($operateThread['print']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-unite'; ?>" class="">印戳</a><?php  } 
  if ($operateThread['move']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-move'; ?>" class="">移动</a><?php  } 
  if ($operateThread['copy']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-copy'; ?>" class="">复制</a><?php  } 
  if ($operateThread['unite']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-unite'; ?>" class="">合并</a><?php  } 
  if ($hasThirdPart && ($hasFirstPart ^ $hasSecondPart || $hasFirstPart && $hasSecondPart)){ ?><i>|</i><?php  } 
  if ($operateThread['lock']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-lock'; ?>" class="">锁定</a><?php  } 
  if ($operateThread['down']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-manage-down'; ?>" class="">压帖</a><?php  } 
  if ($operateThread['delete']){ ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-delete'; ?>">删除</a><?php  } 
  if ($designPermission > 0) { 
  if ($hasFirstPart || $hasSecondPart || $hasThirdPart){ ?><i>|</i><?php  } ?>
				<a data-type="norefresh" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','design-push-add?fromtype=thread&fromid=', rawurlencode($tid); ?>" class="" title="推送">推送</a><!--这玩意交互改下class="mr10 J_manage_single" id="J_push_trigger"  以前的推送class="J_read_push" -->
				<?php  } ?>
			</div>
			<?php  } 
...
<table width="100%" style="table-layout:fixed;" class="floor_table">
  ...
  if ($key == 'uid') { ?>
					...
					<?php  } elseif ($key == 'fans') { ?>
					<li><em>粉丝</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-fans-run?uid=', rawurlencode($read['created_userid']); ?>" target="_blank"><?php echo htmlspecialchars($users[$read['created_userid']]['fans'], ENT_QUOTES, 'UTF-8');?></a></li>
					<?php  } elseif ($key == 'follows') { ?>
					<li><em>关注</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-follows-run?uid=', rawurlencode($read['created_userid']); ?>" target="_blank"><?php echo htmlspecialchars($users[$read['created_userid']]['follows'], ENT_QUOTES, 'UTF-8');?></a></li>
					<?php  } elseif ($key == 'posts') { ?>
					<li><em>发帖数</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','space-thread-run?uid=', rawurlencode($read['created_userid']); ?>" target="_blank"><?php echo htmlspecialchars($users[$read['created_userid']]['postnum'], ENT_QUOTES, 'UTF-8');?></a></li>
 ...
 echo htmlspecialchars($creditBo->cUnit[$key], ENT_QUOTES, 'UTF-8');?></span></li>
				...
				<div class="operate cc">
					<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','my-follow-add'; ?>" data-uid="<?php echo htmlspecialchars($read['created_userid'], ENT_QUOTES, 'UTF-8');?>" class="follow J_read_follow J_qlogin_trigger">加关注</a>
					<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','message-message-pop?username=', rawurlencode($read['created_username']); ?>" class="message J_send_msg_pop J_qlogin_trigger" data-name="<?php echo htmlspecialchars($read['created_username'], ENT_QUOTES, 'UTF-8');?>">写私信</a>
					<?php
...
PwHook::display(array($threadDisplay, "runDo"), array('createHtmlAfterUserInfo',$users[$read['created_userid']], $read), "", $__viewer);
  ...
  if ($read['lou'] == 0) { 
		...
		<td class="box_wrap floor_right">
			...
			<div class="floor_title cc">
				...
				<h1 id="J_post_title"><?php  if ($threadInfo['topic_type'] && ($topicTypename = $threadDisplay->getTopicTypeName($threadInfo['topic_type']))) { ?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','thread/', rawurlencode($pwforum->fid),'.html?type=', rawurlencode($threadInfo['topic_type']); ?>">[<?php echo $topicTypename;?>]</a><?php  } 
  ...
  if ($read['lou'] == 0) { 
				...
				<div id="J_read_moredown_list" class="core_menu" style="display:none;">
					<ul class="core_menu_list">
						<li><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','articles/', rawurlencode($read['tid']),'.html?uid=', rawurlencode($read['created_userid']); ?>">只看楼主</a></li>
						<li><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','articles/', rawurlencode($read['tid']),'.html?desc=1'; 
 echo htmlspecialchars($_urlDescArgs, ENT_QUOTES, 'UTF-8');?>">倒序阅读</a></li>
  ...
  if ($loginUser->uid && ($read['created_userid'] == $loginUser->uid || $threadPermission['edit'])) { ?>
				 <a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-modify?tid=', rawurlencode($tid),'&pid=', rawurlencode($read['pid']); ?>">[编辑]</a>
				<?php  } ?>
				</span>
			</div>
  ...
  if (!empty($read['istopped'])) { ?>
			...
			<div id="J_read_main">
				...
				<div id="J_read_tag_wrap" class="read_tag_list">
					...
					<a data-url="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','tag-index-card?name=', rawurlencode($tag); ?>" class="J_read_tag_item tag_item" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','tag/', rawurlencode($tag),'.html'; ?>"><?php echo htmlspecialchars($tag, ENT_QUOTES, 'UTF-8');?></a>
  ...
  if (($read['created_userid'] === $loginUser->uid && $loginUser->getPermission('tag_allow_add')) || $loginUser->getPermission('tag_allow_edit')) { ?>
				...
				<div id="J_read_tag_edit" class="mb10 cc" style="display:none;">
					<form id="J_read_tag_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','tag-index-editReadTag?tid=', rawurlencode($tid); ?>" method="post">
...
?>
 ...
 echo $read['content'];?>
  ...
  if ($threadPermission['deleteatt']) { ?><a class="J_read_img_del w" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-attach-delete'; ?>" data-pdata="{'aid':'<?php echo htmlspecialchars($value['aid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a><?php  } ?></p></div><a data-big="<?php echo htmlspecialchars($value['url'], ENT_QUOTES, 'UTF-8');?>" href="javascript:;"><?php echo $value['img'];?></a></span></li>
						<?php  } ?>
					</ul>
					<?php  if ($_isShowSmall) { ?>
 ...
 echo htmlspecialchars($creditBo->cType[$value['ctype']], ENT_QUOTES, 'UTF-8');?></span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-attach-record?aid=', rawurlencode($value['aid']); ?>" title="查看记录" class="mr10 fn J_buy_record" data-aid="<?php echo htmlspecialchars($value['aid'], ENT_QUOTES, 'UTF-8');?>">[记录]</a>
							<?php  } 
  if ($threadPermission['deleteatt']) { ?><a class="J_attach_post_del" data-pdata="{'aid':'<?php echo htmlspecialchars($value['aid'], ENT_QUOTES, 'UTF-8');?>'}" data-rel="#J_att_<?php echo htmlspecialchars($value['aid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-attach-delete'; ?>">[删除]</a><?php  } ?>
						</span>
					</dd>
					<?php  } ?>
				</dl>
...
PwHook::display(array($threadDisplay, "runDo"), array('createHtmlAfterContent',$read), "", $__viewer);
  ...
  if ($read['lou'] == 0) { ?>
			<div class="read_appbtn_wrap">
				<?php  if (!$read['pid']) { ?>
				<a role="button" rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','like-mylike-doLike'; ?>" data-tid="<?php echo htmlspecialchars($tid, ENT_QUOTES, 'UTF-8');?>" data-pid="<?php echo htmlspecialchars($read['pid'], ENT_QUOTES, 'UTF-8');?>" data-fid="<?php echo htmlspecialchars($fid, ENT_QUOTES, 'UTF-8');?>" class="icon_read_like J_like_btn J_qlogin_trigger" data-role="main" data-typeid="1" data-fromid="<?php echo htmlspecialchars($read['tid'], ENT_QUOTES, 'UTF-8');?>">
  ...
  if ($read['lou'] == 0) { 
			...
			<div class="floor_bottom_tips cc">
				...
				<span class="fr">
					...
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-read-log?tid=', rawurlencode($read['tid']),'&fid=', rawurlencode($read['fid']); ?>" class="mr10 J_qlogin_trigger" id="J_inside_logs">查看操作记录</a>
					<?php }
  if ($operateReply['toppedreply'] && $read['lou']){ 
  if (!$read['topped']){ ?>
					<a href="#" data-uri="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-dotoppedreply?topped=1'; ?>" data-pdata="{'tid':'<?php echo htmlspecialchars($read['tid'], ENT_QUOTES, 'UTF-8');?>', 'lou':'<?php echo htmlspecialchars($read['lou'], ENT_QUOTES, 'UTF-8');?>', 'pid':'<?php echo htmlspecialchars($read['pid'], ENT_QUOTES, 'UTF-8');?>'}" class="mr10 J_post_top J_qlogin_trigger">帖内置顶</a>
						<?php  } else { ?>
					<a href="#" data-uri="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-dotoppedreply?topped=0'; ?>" data-pdata="{'tid':'<?php echo htmlspecialchars($read['tid'], ENT_QUOTES, 'UTF-8');?>', 'lou':'<?php echo htmlspecialchars($read['lou'], ENT_QUOTES, 'UTF-8');?>', 'pids':'<?php echo htmlspecialchars($read['pid'], ENT_QUOTES, 'UTF-8');?>'}" class="mr10 J_post_top J_qlogin_trigger">取消置顶</a>
						<?php  } 
  } 
  if ($operateReply['read']){ ?>
					<a href="#" data-uri="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-doinspect'; ?>" data-pdata="{'tid':'<?php echo htmlspecialchars($read['tid'], ENT_QUOTES, 'UTF-8');?>', 'lou':'<?php echo htmlspecialchars($read['lou'], ENT_QUOTES, 'UTF-8');?>','pids[]':'<?php echo htmlspecialchars($read['pid'], ENT_QUOTES, 'UTF-8');?>'}" class="mr10 J_read_mark J_qlogin_trigger">已阅</a>
					<?php  } 
  if ($loginUser->getPermission('allow_report')) { ?>
					<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','report-index-report?type=', rawurlencode($type),'&type_id=', rawurlencode($type_id); ?>" data-pid="<?php echo htmlspecialchars($read['pid'], ENT_QUOTES, 'UTF-8');?>" class="mr10 J_report J_qlogin_trigger">举报</a>
					<?php  } 
  if ($operateReply) { ?>
...
<?php } ?>
			<div class="read_pages_wrap cc" id="floor_reply">
				<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-run?fid=', rawurlencode($pwforum->fid); ?>" id="J_read_post_btn" class="btn_post<?php echo htmlspecialchars($postNeedLogin, ENT_QUOTES, 'UTF-8');?>">发帖</a>
				<!-- 锁定时间 -->
				<?php  if($showReply){ ?>
				<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-reply?tid=', rawurlencode($tid); ?>" data-referer="true" class="btn_replay<?php echo htmlspecialchars($replyNeedLogin, ENT_QUOTES, 'UTF-8');?>">回复</a>
				<?php  } ?>
...
<?php } ?>
  ...
  $_urladd_ = ($key != 'default') ? ('&special=' . $key) : ''; ?>
					<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-run?fid=', rawurlencode($pwforum->fid); 
						...
						<td class="floor_reply box_wrap">
							...
							<?php  if (!$loginUser->isExists()) { ?>
								<div class="reply_login_tips">
									您需要登录后才可以回帖，<a href="#floor_reply" rel="nofollow" class="J_qlogin_trigger">登录</a>&nbsp;或者&nbsp;<a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','u-register-run'; ?>">注册</a>
								</div>
							<?php  } else { ?>
							<div class="reply_toolbar_wrap">
								<div class="reply_toolbar cc">
									<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-reply?tid=', rawurlencode($tid); ?>" class="reply_high">进入高级模式&gt;&gt;</a>
							...
							<div class="J_reply_ft" id="J_reply_ft">
								<button type="submit" data-tid="<?php echo htmlspecialchars($tid, ENT_QUOTES, 'UTF-8');?>" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-post-doreply?_getHtml=1'; ?>" class="btn btn_submit disabled" disabled="disabled" id="J_reply_quick_btn">回复</button>
							</div>
							<?php  } ?>
						</td>
					</tr>
					...
					<div class="pop_cont">
						<div class="pop_operat_list">
							<ul class="cc J_post_manage_col" data-role="read">
								<?php  if ($operateReply['delete']){ ?><li><a data-type="delete" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-delete'; ?>">删除</a></li><?php  } 
  if ($operateReply['unite']){ ?><li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-unite'; ?>">合并</a></li><?php  } 
  if ($operateReply['split']){ ?><li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-split'; ?>">拆分</a></li><?php  } 
  if ($operateReply['shield']){ ?><li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-shield'; ?>">屏蔽</a></li><?php  } 
  if ($operateReply['remind']){ ?><li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','bbs-masingle-remind'; ?>">提醒</a></li><?php  } ?>
							</ul>
						</div>
					</div>
				</div>
